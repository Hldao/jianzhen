...
**进度：v1.1.0 全部代码就绪，等待你在微信开发者工具里上传 + mp 后台提交审核。**

| 维度 | 状态 |
|---|---|
| 代码 | ✅ 已 push 至 `Hldao/jianzhen` main |
| 隐私协议（mp 后台） | ✅ 5 条接口已配置 |
| 数据库索引 | ✅ 2 个关键索引已建 |
| 云函数部署 | ✅ training 含 updateMedias 已上传 |
...
```
1. 微信开发者工具 → 右上角「上传」→ 版本号 1.1.0 → 提交
2. mp.weixin.qq.com → 版本管理 → 开发版本 → 找到 1.1.0 → 点「提交审核」
3. 填功能页面信息：首页选 /pages/index/index
4. 填测试账号：「无需测试账号，微信一键登录」
5. 等待审核（一般 1-3 个工作日）
6. 通过后 mp 后台 → 版本管理 → 「审核版本」点「发布」
```
